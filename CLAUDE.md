...
- **Backup flow:** reveal seed behind an explicit "I understand" gate; require the user to re-enter / confirm a subset of words before marking backed-up. **Screenshots are intentionally NOT blocked** (decided 2026-06-25 — capturing one's own recovery phrase is the user's call; the UI still advises against it). The app-switcher snapshot stays obscured (`obscuredWhenBackgrounded` on iOS); the `PlatformBridge.setSecureScreen` / `FLAG_SECURE` capability remains in the codebase but is no longer applied to the seed (Backup/Import) screens.
...
**Backup wallet** — gated reveal of the selected wallet's mnemonic (biometric/passcode), followed by a verify step (confirm N random words). Screenshots are not blocked (the user's call); see §7. Mark `isBackedUp` for that wallet on success; clear its banner.
...
**Send** — scan/paste address (parse BIP21), enter amount (with max), pick fee tier, review screen (**network** + recipient, amount, fee, total), confirm → `TxBuilder`→`Psbt`→`sign`→`broadcast`. Handle insufficient-funds, dust, and broadcast errors explicitly. Optimistically insert the tx into the selected wallet's history as pending. **Send to one of my wallets** (2026-07-28): the recipient step lists the user's OTHER wallets **on the same network** and fills the address from `nextUnusedAddress` (never `nextReceiveAddress` — browsing the picker must not burn index space). Same-network is a safety filter, not a convenience: `.ecash` uses Bitcoin's `bc` HRP, so a cross-network entry would look legitimate and send coins to a chain that will never see them.

**Split coins** (eCash only) — drain the whole spendable balance to a FRESH address of the same
wallet, separating fork-airdrop coins from the Bitcoin they share UTXOs with. Home nudge + Settings
row appear only when `splitSummary.needsSplitCount > 0`. **PROVEN ON-CHAIN 2026-07-28** on drynet3
with a real pre-fork P2PKH UTXO imported by WIF. **Self-transfers display specially:** a split's
`netSats` is only the fee (it pays nobody), so rows show `WalletTx.receivedSats` — the amount that
moved — labelled "Sent to yourself". Netting the fee out, as a normal send does, renders a
whole-balance move as `0.00000000`.
...
- **CoinNews** — the on-chain (`OP_RETURN`) bulletin board (L2L). **Substantially BUILT** (per-network; News tab hidden on Bitcoin). Reads the public `coinnews.v1` indexer (`CoinNewsEndpointRegistry`; signet → `https://coinnews.signet.dc.galaxoidlabs.com`) — feed (Top/Latest), topics, follow/filter, threads. Publishes **stories, topics, comments, votes** plus **replies-to-comments and comment votes** as signed `OP_RETURN`s (BIP-340 identity per wallet, `CoinNewsMessage`/`CoinNewsCrypto`); reuses the BDK tx path (`WalletManager.publish*` → `WalletEngine.publishData`). Optimistic local copies (`PendingCoinNewsStore`) until indexed; votes are **first-wins** (locked after a successful vote). CoinNews txs are marked in Activity and get a kind-header + collapsible-detail sheet. Story detail = chat-style composer, vote pill, threaded comments. Still TODO: trust-minimized client-side sig/ItemID verification; richer tx-detail content (decoded comment text — see memory). Full design/status in `docs/coinnews-integration.md`.
